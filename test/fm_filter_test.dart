...
void main() {
  ...
  group('tür, kaynak ve ad', () {
    ...
    test('kaynak süzgeci yola bakar', () {
      final filter = FmFilter.none.withBucket(MediaBucket.whatsapp);
      expect(
          filter.matches(entry('/storage/WhatsApp/Media/a.mp4'), now: now),
          isTrue);
      expect(filter.matches(entry('/storage/DCIM/b.mp4'), now: now), isFalse);
    });
